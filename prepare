...
if __name__ == '__main__':
    ...
    with open('{}train_mean_std.pkl'.format(cfg['train_data_dir']), 'wb') as f:
        pickle.dump([mean, std], f)
    print('mean:{}, std:{}'.format(mean, std))
    dataset1 = SEIDataset(data_file=cfg['h5_file'], split='test') 
    mean1, std1 = get_mean_std(dataset1, ratio=1)
    with open('{}test_mean_std.pkl'.format(cfg['test_data_dir']), 'wb') as f:
        pickle.dump([mean1, std1], f)
    print('mean:{}, std:{}'.format(mean1, std1))
